...
fn main() {
    let lock_file = Lockfile::load("../Cargo.lock").expect("cargo lock file should exist before the build kicks off");

    let egui_theme_pkg = lock_file
        .packages
        .iter()
        .find(|item| item.name.as_str() == PACKAGE_NAME ).expect("this must exist");

    let egui_pkg = egui_theme_pkg.dependencies
        .iter()
    ...
    let out_file =
        File::create("generated/meta.rs").expect("cannot create file");

    let mut writer = BufWriter::new(out_file);
    writeln!(writer, "// THIS IS AN AUTOGENERATED FILE AND SHOULD NOT BE MODIFIED OR SAVED").expect("failed to write");
