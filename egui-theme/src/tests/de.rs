...
fn test_serialize_default() {
    ...
    assert_eq!(deserialized_theme.egui_theme_version, crate::EGUI_THEME_VERSION, "egui_theme_version should match");
    assert_eq!(deserialized_theme.egui_version, crate::EGUI_VERSION, "egui_version should match");
...
fn test_deserialize_custom() {
    ...
    assert_eq!(deserialized_theme.egui_version, crate::EGUI_VERSION, "egui_version should match");
    let (style, fonts) = deserialized_theme.extract();
    assert_eq!(style.spacing.slider_width, 100f32, "spacing.slider_width should be 100");
    assert_eq!(style.visuals.popup_shadow.color, egui::Color32::from_rgba_premultiplied(0, 0, 0, 96), "spacing.slider_width should be 100");
    assert_eq!(style.spacing.slider_width, 100f32, "spacing.slider_width should be 100");
    assert_eq!(style.visuals.clip_rect_margin, 3f32, "visuals.clip_rect_margin should be 3");
    assert_eq!(style.override_font_id, None, "Override font should be None");
    assert_eq!(style.visuals.faint_bg_color, egui::Color32::from_rgba_premultiplied(35,35,35,255), "color should be (35, 35, 35, 255)");

    assert!(fonts.font_data.contains_key("Nacelle-Regul"), "font data should contain Nacelle-Regul");
