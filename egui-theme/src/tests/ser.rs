...
fn test_custom_font_extract() {
    ...
    font_definitions.font_data.insert(FONT_NAME.to_owned(), FontData::from_static(include_bytes!("test-fonts/Nacelle-Regular.otf")));
    let result = font_definitions.families.insert(egui::FontFamily::Name(FONT_NAME.into()), vec![FONT_NAME.to_owned()]);
    ...
    assert!(fonts.font_data.contains_key(&FONT_NAME.to_owned()), "does not have `Nacelle` key");
    assert_eq!(fonts.font_data.get(&FONT_NAME.to_owned()).unwrap(), &FontData::from_static(include_bytes!("test-fonts/Nacelle-Regular.otf")), "font data does not match");
    assert!(fonts.families.contains_key(&egui::FontFamily::Monospace), "does not have the `Monospace` family");
    assert!(fonts.families.contains_key(&egui::FontFamily::Proportional), "does not have the `Proportional` family");
    assert!(fonts.families.contains_key(&egui::FontFamily::Name(FONT_NAME.into())), "does not have the `Nacelle` family");
    assert_eq!(fonts.families.get(&egui::FontFamily::Name(FONT_NAME.into())).unwrap(), &vec![FONT_NAME.to_owned()], "`Nacelle` family does not have the correct data");
}
...
fn test_custom_font_serialization() {
    ...
    let mut font_definitions = egui::FontDefinitions::default();
    font_definitions.font_data.insert(FONT_NAME.to_owned(), FontData::from_static(include_bytes!("test-fonts/Nacelle-Regular.otf")));
    font_definitions.families.insert(egui::FontFamily::Name(FONT_NAME.into()), vec![FONT_NAME.to_owned()]);
    ...
    assert!(serialized.contains("Nacelle"), "Nacelle not found in the serialized string");
    let nacelle_font_data = serde_json::to_value(FontData::from_static(include_bytes!("test-fonts/Nacelle-Regular.otf"))).unwrap();
    assert!(serialized.contains(&nacelle_font_data.to_string()), "Nacelle data does not exist in the serialized string");
}